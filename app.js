...
const ROTATION_WEIGHTS = { A: 4, B: 2, C: 1, D: 0.33 };
...
const COLUMN_ALIASES = {
  ...
  rotation: ['rotation','visit class','frequency','abcdrank','route class']
};
...
function bindElements() {
  [
    ...
    'assign-rep-select','rep-filter','rep-count-input','disruption-slider','disruption-value','balance-mode','rotation-filter',
...
function bindEvents() {
  ...
  els.rotationFilter.addEventListener('change', refreshUI);
  ...
  els.disruptionSlider.addEventListener('input', () => { els.disruptionValue.textContent = els.disruptionSlider.value; });
}
...
function loadWorkbook(workbook) {
  ...
  els.sheetSelect.disabled = workbook.SheetNames.length <= 1 ? false : false;
  enableTopControls();
  loadSelectedSheet();
}
...
function normalizeRows(rows) {
  ...
  for (const row of rows) {
    ...
    const rotation = normalizeRotation(row[headerMap.rotation], row[headerMap.rank]);
    ...
    mapped.push({
      ...
      rank: safeString(row[headerMap.rank]),
...
function buildRepColors() {
  ...
  reps.forEach((rep, idx) => state.repColors.set(rep, COLOR_PALETTE[idx % COLOR_PALETTE.length]));
}
...
function syncControlState() {
  ...
  [
    ...
    els.assignRepSelect, els.repFilter, els.repCountInput, els.disruptionSlider,
    els.balanceMode, els.rotationFilter, els.dimOthersCheckbox, els.showTerritoryCheckbox, els.clearSelectionBtn
  ].forEach(el => { el.disabled = !hasData || (el === els.assignBtn && state.selection.size === 0); });
...
function renderRepControls() {
  ...
  if (!els.repCountInput.dataset.userTouched) els.repCountInput.value = suggestedCount;
  els.repCountInput.oninput = () => { els.repCountInput.dataset.userTouched = '1'; };
}
...
function markerStyleForAccount(account) {
  ...
  const rotationFilter = els.rotationFilter.value || 'ALL';
  ...
  const rotPass = rotationFilter === 'ALL' || account.rotation === rotationFilter;
  const visible = repPass && rotPass;
...
function buildPopupHtml(account) {
  return `
    <div style="min-width:220px;">
      ...
      <div style="font-size:12px;color:#5d7286;margin:4px 0 8px;">${escapeHtml(account.customerId)}</div>
      ...
      <div><strong>Rotation:</strong> ${escapeHtml(account.rotation)}</div>
...
function renderSelectionPreview() {
  ...
  els.selectionPreview.innerHTML = selected.map(a => `
    <div class="preview-item">
      ...
      <div class="preview-meta">${escapeHtml(a.customerId)} • ${escapeHtml(a.assignedRep)} • ${formatCurrency(a.overallSales)} • ${a.rotation}${a.protected ? ' • Protected' : ''}</div>
    </div>
  `).join('');
  syncControlState();
}
...
function renderSummary() {
  ...
  const workload = state.accounts.reduce((sum, a) => sum + (ROTATION_WEIGHTS[a.rotation] || 1), 0);
...
function assignSelectionToRep() {
  ...
  clearSelection();
  if (skippedProtected) showToast(`${changes.length} changed. ${skippedProtected} protected account(s) skipped.`);
}
...
function applyChanges(changes, label) {
  ...
  state.undoStack.push({ changes: changes.map(c => ({ ...c })), label });
  buildRepColors();
  refreshUI();
...
function optimizeRoutes() {
  ...
  const locked = state.accounts.filter(a => a.protected);
  ...
  targetRepNames.forEach(rep => {
    if (!state.repColors.has(rep)) state.repColors.set(rep, COLOR_PALETTE[state.repColors.size % COLOR_PALETTE.length]);
  });

  const centroids = initializeCentroids(targetRepNames, currentReps);
  const assignments = new Map();

  locked.forEach(a => assignments.set(a._id, a.assignedRep));

  for (let iter = 0; iter < 5; iter += 1) {
    const repStats = buildEmptyRepStats(targetRepNames);
    locked.forEach(acc => addToRepStats(repStats.get(acc.assignedRep) || repStats.values().next().value, acc));
    ...
    for (const account of movable) {
      ...
      for (const rep of targetRepNames) {
        ...
        const disruptionPenalty = account.currentRep === rep ? (1 - disruptionWeight) * 0 : disruptionWeight * 0.45;
        const sameAssignedPenalty = account.assignedRep === rep ? 0 : disruptionWeight * 0.15;
        const repStat = repStats.get(rep);
        const stopPenalty = balanceMode === 'revenue' ? 0 : repStat.stops * 0.0018;
        const revenuePenalty = balanceMode === 'stops' ? 0 : repStat.revenue * 0.0000009;
        const totalScore = dist + disruptionPenalty + sameAssignedPenalty + stopPenalty + revenuePenalty;
      ...
      addToRepStats(repStats.get(bestRep), account);
    }
  ...
  applyChanges(changes, `Optimized routes to ${targetRepNames.length} reps`);
}

function initializeCentroids(targetRepNames, currentReps) {
  ...
  targetRepNames.forEach((rep, idx) => {
    ...
    centroids.set(rep, { lat: fallbackSource.latitude, lng: fallbackSource.longitude });
  });
...
function refreshTerritories() {
  ...
  reps.forEach(rep => {
    const members = state.accounts.filter(a => a.assignedRep === rep && (rotationFilter === 'ALL' || a.rotation === rotationFilter));
  ...
  for (const account of state.accounts) {
    if (!map.has(account.assignedRep)) {
      map.set(account.assignedRep, {
        ...
        stops: 0, revenue: 0, wine: 0, spirits: 0, thc: 0, A: 0, B: 0, C: 0, D: 0, protected: 0, movedIn: 0, movedOut: 0, workload: 0
      });
    }
    ...
    row[account.rotation] = (row[account.rotation] || 0) + 1;
    row.workload += ROTATION_WEIGHTS[account.rotation] || 1;
...
function exportWorkbook() {
  ...
  const changeLog = state.changeLog.length ? state.changeLog : [{ timestamp: '', customerId: '', customerName: '', fromRep: '', toRep: '', protected: '' }];
...
function enableTopControls() {
  [els.sheetSelect, els.loadSheetBtn].forEach(el => el.disabled = false);
}

function getAllReps() {
  const set = new Set();
  state.accounts.forEach(a => { if (a.assignedRep) set.add(a.assignedRep); if (a.currentRep) set.add(a.currentRep); });
...
function normalizeRotation(rotationValue, rankValue) {
  const raw = safeString(rotationValue || rankValue).toUpperCase();
